...
### RIGHT SIDE PROMPT ###
# Custom additions by Dylan, adds a timestamp and a clearly marked nonzero
# return value

prompt_nonzero_return() {
  if [[ $RETVAL -ne 0 ]]; then
    prompt_segment default red "[$RETVAL]"
  fi
}
...
prompt_timestamp() {
  ...
  # Midnight is 0000, so it's cleaner to do less than or equal to on the minute
  # before. (`-lt 2400` also works here, of course).
